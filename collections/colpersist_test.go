...
func TestColSegmentMarshalRoundTrip(t *testing.T) {
	...
	for _, codec := range []Codec{identityCodec{}, mustZSTD(t)} {
		blk := encodeColumnarBlock(s, recs, resolveColLayout(s, hotHalf(s)), codec, nil)
		...
		gb := got.blocks[0]
		if gb.n != blk.n || gb.bitsStride != blk.bitsStride || len(gb.hotFields()) != len(blk.hotFields()) || len(gb.coldFields()) != len(blk.coldFields()) {
...
func TestUnmarshalColSegmentTruncated(t *testing.T) {
	...
	blk := encodeColumnarBlock(s, encodeRows(s, wires), resolveColLayout(s, nil), identityCodec{}, nil)
...
func TestColSegmentSchemaNameAnchored(t *testing.T) {
	...
	blk := encodeColumnarBlock(s, encodeRows(s, wires), resolveColLayout(s, hotHalf(s)), identityCodec{}, nil)
