...
func TestGroupBlockMembershipAndExceptions(t *testing.T) {
	...
	gbs := buildGroupBlocks([]*colGroup{g}, []*colLayout{resolveColLayout(g.schema, nil)}, iws, c.regionCodec(), nil)
	if len(gbs) != 1 {
...
func TestGroupBlockIndexIsRank(t *testing.T) {
	...
	iws, ids, state := groupBlockFixture(t, c, n)
	g := mkGroup(t, c, iws, ids)
	gb := buildGroupBlocks([]*colGroup{g}, []*colLayout{resolveColLayout(g.schema, nil)}, iws, c.regionCodec(), nil)[0]
