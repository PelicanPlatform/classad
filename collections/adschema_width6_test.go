...
func TestIntWidth6Byte(t *testing.T) {
	...
	blk := encodeColumnarBlock(s, encodeRows(s, wires), resolveColLayout(s, []int{s.byID[bigID]}), identityCodec{}, nil)
...
func BenchmarkIntWidth6vs8(b *testing.B) {
	...
	blk6 := encodeColumnarBlock(s, encodeRows(s, wires), resolveColLayout(s, []int{s.byID[bigID]}), codec, nil)
	blk8 := encodeColumnarBlock(s8, encodeRows(s8, wires), resolveColLayout(s8, []int{s8.byID[bigID]}), codec, nil)
